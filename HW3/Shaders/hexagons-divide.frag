...
const float a   = 0.1; //short = 1.732 (sqrt(3)) * a * 0.5 || long = a
...
void main()
{
   ...
   //  Scroll bricks at 0.25 hexagons per second
   position.y += 0.25*time;
   //  Don't care about the integer part
   position = fract(position);
   //  make the hexagon outline in seperate color with sharp divide
   //right side
   vec2 use1a1 = step(position,vec2(1,1 - 1.0/6.0));
   vec2 use1a2 = step(-position,-vec2(1 - (border/2),3*1.0/6.0 - 0.01)); //invert for greater than

   //right top
   vec2 use2a1 = step(position,vec2(1.0, -(2*1.0/6.0)*position.x + 1 + 1.0/6.0 + vertborder/4)); //y=mx+b
   vec2 use2a2 = step(-position,-vec2(0.5, -(2*1.0/6.0)*position.x + 1 + 1.0/6.0 - 3*vertborder/4));

   //right bottom
   vec2 use3a1 = step(position,vec2(1.0, 2*1.0/6.0*position.x -1.0/6.0 + 2*1.0/6.0 + vertborder/4));
   vec2 use3a2 = step(-position,-vec2(0.5, 2*1.0/6.0*position.x -1.0/6.0 + 2*1.0/6.0 -3*vertborder/4));
   
   //left
   vec2 use4a1 = step(position,vec2(border/2,1 - 1.0/6.0));
   vec2 use4a2 = step(-position,-vec2(0,3*1.0/6.0 - 0.01));
   
   //left top
   vec2 use5a1 = step(position,vec2(0.5, 2*1.0/6.0*position.x + 1-1.0/6.0 + vertborder/4));
   vec2 use5a2 = step(-position,-vec2(0.0, 2*1.0/6.0*position.x + 1-1.0/6.0- 3*vertborder/4));
   
   //left bottom
   vec2 use6a1 = step(position,vec2(0.5, (-2*1.0/6.0)*position.x + 3*1.0/6.0 + vertborder/4));
   vec2 use6a2 = step(-position,-vec2(0.0, (-2*1.0/6.0)*position.x + 3*1.0/6.0 - 3*vertborder/4));

   //bottom
   vec2 use7a1 = step(position,vec2(0.5+border/2,2*1.0/6.0));
