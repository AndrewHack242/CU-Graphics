...
void main()
{
   ...
   //right side
   vec2 use1a1 = dostep(position,vec2(1,1 - calca));
   vec2 use1a2 = dostep(-position,-vec2(1 - (border/2),3*calca - 0.01)); //invert for greater than

   //right top
   vec2 use2a1 = dostep(position,vec2(1.0, -(2*calca)*position.x + 1 + calca + vertborder/4)); //y=mx+b
   vec2 use2a2 = dostep(-position,-vec2(0.5, -(2*calca)*position.x + 1 + calca - 3*vertborder/4));

   //right bottom
   vec2 use3a1 = dostep(position,vec2(1.0, 2*calca*position.x -calca + 2*calca + vertborder/4));
   vec2 use3a2 = dostep(-position,-vec2(0.5, 2*calca*position.x -calca + 2*calca -3*vertborder/4));
   
   //left
   vec2 use4a1 = dostep(position,vec2(border/2,1 - calca));
   vec2 use4a2 = dostep(-position,-vec2(0,3*calca - 0.01));
   
   //left top
   vec2 use5a1 = dostep(position,vec2(0.5, 2*calca*position.x + 1-calca + vertborder/4));
   vec2 use5a2 = dostep(-position,-vec2(0.0, 2*calca*position.x + 1-calca- 3*vertborder/4));
   
   //left bottom
   vec2 use6a1 = dostep(position,vec2(0.5, (-2*calca)*position.x + 3*calca + vertborder/4));
   vec2 use6a2 = dostep(-position,-vec2(0.0, (-2*calca)*position.x + 3*calca - 3*vertborder/4));
